	...
	private void checkAttacks() {
		...
		for(Entity e : handler.getWorld().getEntityManager().getEntities()){
			if(e.equals(this))
				continue;
			//search
			if(e.getCollisionBounds(0, 0).intersects(searchArea)&&e.equals(handler.getWorld().getEntityManager().getPlayer())){
				attackMode = true;
			}else {
				tickCount++;
		        if(tickCount==120) {
			...
			//attack
			if(e.getCollisionBounds(0, 0).intersects(attackArea)&&e.equals(handler.getWorld().getEntityManager().getPlayer())){
				e.hurt(1);
			}
		}
	}
